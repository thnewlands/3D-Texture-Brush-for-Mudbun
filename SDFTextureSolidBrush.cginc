...
float _MudbunSDFTexturesPerDimension;
float _MudbunSDFTextureResolution;
...
float sdf_texture3D_brush(SdfBrush brush, float3 pRel)
{
    ...
    float insetFromBorder = 1 / (_MudbunSDFTextureResolution * _MudbunSDFTexturesPerDimension);
    float sampleRadius = .1;
            
    float3 boundsMin = origin + insetFromBorder;
    float3 boundsMax = origin + (1.0 / _MudbunSDFTexturesPerDimension) - insetFromBorder;
